...
|---|---|
| **Privacy** | Inference stays on-device. Network bind defaults to loopback; non-loopback requires an explicit choice and confirmation. Listing/refreshing the model catalog does contact Microsoft's Foundry Local catalog over the network (the startup check can be turned off in Settings). |
