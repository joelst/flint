...
- **Endpoint and agent integration gate:** User-facing self-test verifies `/v1/models` envelope, then model ID reuse in chat completions, streaming termination with `[DONE]`, disconnect cancellation, `usage` when the model emits it, and either valid `tool_calls` or an explicit not-verified label, for every listed id and parent alias. Embedding ids get an embeddings check and speech ids get a transcription check. Continue, Cline, and OpenClaw recipes are pinned to tested client versions.
