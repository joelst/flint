...
- Sidecar `embedTexts` uses `EmbeddingsSession` through `sidecar/embeddings-session.js` with the same pool inFlight fencing as chat. Batches are bounded (32 strings, 8k chars each).
