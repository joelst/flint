...
- Listing/refreshing the catalog contacts Microsoft's Foundry Local model catalog over the network (separate from inference, which stays local). Flint checks it on startup by default; disable **Settings → Startup → "Check model catalog on startup"** to skip recommendations and configured model preloads for that launch. **Refresh catalog** later populates the Models view but does not replay startup-only work. Accelerator setup is separate: Flint runs it automatically at startup, and Foundry Local may download execution-provider components when needed. **Recheck Providers** on the Models page removes the cache for a provider that is not registered and downloads that provider again. When every provider is already registered, it does not download anything. A provider whose files are still loaded, so Windows will not delete them, is left out of that download. The status line names any provider that is still not registered afterward. Model actions such as download or import may also require network access or refresh the catalog afterward.
