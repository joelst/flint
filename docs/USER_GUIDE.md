...
- Listing/refreshing the catalog contacts Microsoft's Foundry Local model catalog over the network (separate from inference, which stays local). Flint checks it on startup by default; disable **Settings → Startup → "Check model catalog on startup"** to skip recommendations and configured model preloads for that launch. **Refresh catalog** later populates the Models view but does not replay startup-only work. Accelerator setup is separate: Flint runs it automatically at startup, and Foundry Local may download execution-provider components when needed. **Recheck Providers** on the Models page asks Foundry Local to register each execution provider that is not registered. CUDA and WebGPU are downloaded into Flint's own cache, so for those two it removes the cached copy first and Foundry Local downloads a new one. The other providers (QNN, OpenVINO, DirectML, NvTensorRTRTX) come from Windows ML or the runtime and have no Flint cache; Recheck only registers them again, which can still fetch their package. When every provider is already registered, it does nothing. A CUDA or WebGPU cache whose files are still loaded, so Windows will not delete them, is left in place and not downloaded again. The status line names any provider that is still not registered afterward. Model actions such as download or import may also require network access or refresh the catalog afterward.
...
**Diagnostics → Test local endpoint** checks the local gateway's envelope, then runs a chat round-trip, streaming `[DONE]` termination, `usage`, and tool-call behavior for every listed chat id and parent alias. Each listed embedding id gets an embeddings check, and each listed speech id gets a transcription check. After all ids for one alias are tested, Flint puts that alias back the way it was just before its first probe: a model that was resident then is reloaded to that exact variant, and one that was not is unloaded. While the test runs, loading or unloading models from Models, Monitor, the chat model switch, or the Arena is refused, and the test does not start while one of those is in progress. The sidecar fences that guarded unload atomically, so new gateway or IPC work is rejected before it can race model teardown. If restoration fails or new activity makes an unload unsafe, the remaining probes are blocked instead of loading more models. A single endpoint-level disconnect check runs after every other model probe because aborting the client stream does not prove that native generation has stopped, so it is not reported as per-model verification. That terminal chat model can remain resident when no chat model was resident before the test; unloading it immediately could interrupt native generation that outlived the client abort. Speech checks prepare the model first and submit its loaded variant because multipart transcription requests cannot be gateway-replayed. The embeddings check is blocked when no embedding-capable model is cached — a blocked check is not evidence embeddings work end to end (see BYOM above).
