...
- Transcript timestamps are approximate and generated by Flint's audio segmentation,
  using silence detection when available and fixed-length windows otherwise. SRT has no
  portable metadata header, so download its timing note separately; WebVTT includes the
  note in the file.
