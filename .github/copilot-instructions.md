...
- SDK 2.0.1 fixes the catalog on first access and no-argument EP registration selects only one preferred provider. Register every discovered EP explicitly before the first catalog read; otherwise aliases without that provider's build silently lose their other GPU variants for the process.
