...
- Provider discovery is ordered behind earlier provider registration, but not behind unrelated catalog mutations. Automatic accelerator setup is single-flight; an explicit Install / Update request waits for the active cycle and then performs a fresh rerun.
