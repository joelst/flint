...
- Only **cached** models are resolvable for autoload (`sidecar/model-registry.js`), so a stray identifier cannot start a download. Call `invalidateModelIndex()` wherever the cached set changes. Gateway loading rechecks `isCached` under the alias residency lock after resolution, and again before reloading a resident entry that telemetry reports as no longer loaded, so deletion cannot race resolution into loading a removed build.
...
- Proxied requests never reach the sidecar's chat path, so the pool cannot see them. `createGateway({onActivity})` keeps each whole exchange (buffer → forward → autoload → replay) continuously booked; when resolution or loading changes the serving identity, it hands the lease off with per-variant start/end pairs. Without it a model streaming a long completion looks idle and can be evicted mid-generation.
- Multipart transcription requests are not replayable. The endpoint self-test preloads the target and submits the canonical loaded variant. The gateway peeks only the bounded leading `model` field on `/v1/audio/transcriptions`, forwards every consumed byte before piping the remainder, and carries the start booking token to the matching end so speech work is not evicted or charged to the wrong usage key.
...
- Flint's long-audio timestamps are therefore **derived source-window estimates**, never model output. `src/lib/audio-segmentation.ts` plans approximately 28 s windows, snaps eligible interior boundaries to detected pauses, and uses a 4 s overlap after hard fixed-window splits where no usable pause exists.
- `src/lib/long-audio-transcript.ts` performs text-only heuristic stitching by removing the longest word overlap between adjacent successful overlapping windows. A successfully processed window whose entire recognition is removed is retained as an overlap-only/no-new-text source outcome; it is not a failure, empty recognition, or proof of silence, and it must not stretch an earlier caption.
- Timestamp exporters receive the complete assembled transcript state. WebVTT `NOTE`, timestamped clipboard text, and the SRT companion `.timing.txt` preserve raw failed, uncertain, unprocessed, empty-recognition, and overlap-only source ranges; diagnostic ranges may overlap and must not be clamped like subtitle cues. Only actual SRT/WebVTT speech cues are normalized to non-overlapping playback ranges.
- Long-audio completion is qualified whenever any window failed, was unprocessed, recognized no text, or contributed overlap-only text. Failed and uncertain chunks remain incomplete; successfully processed empty or overlap-only windows remain distinct non-failure outcomes.
