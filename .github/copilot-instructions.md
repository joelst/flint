...
- Activity accounting is also request admission: destructive unload, variant-switch, eviction, and deletion hold a synchronous per-alias activity fence for their whole destructive window. Gateway and IPC inference must acquire activity before loading or creating temporary state; a point-in-time `inFlight` check is not a fence.
...
- Provider discovery is ordered behind earlier provider registration, but not behind unrelated catalog mutations. Because provider installation is unbounded, `getEps` is unbounded too and uses a 60-second no-progress notice rather than discarding a late result. Automatic accelerator setup is single-flight; an explicit Install / Update request waits for the active cycle and then performs a fresh rerun.
