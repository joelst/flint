...
- Gateway responses buffered for `/status` rewriting or first-pass model-not-loaded inspection are capped before concatenation. Declared or streamed overflow returns 502 and cannot trigger autoload; bodyless HTTP responses retain their semantics. Ordinary and replayed inference responses remain streamed and uncapped.
