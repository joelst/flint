...
- Autoload is reactive: forward first, and retry once after loading only when Foundry rejects the model this request named: `400 Model 'X' must be loaded before inference` (SDK 1.x: `is not loaded`) for a known variant, or `404 Model not found: No model matching 'X'` for an alias or versionless id, and only when the cached-model registry resolves `X`. Never check "is it loaded" up front, and never retry twice.
- Only **cached** models are resolvable for autoload (`sidecar/model-registry.js`), so a stray identifier cannot start a download. Call `invalidateModelIndex()` wherever the cached set changes. A gateway load (`ensureModel(..., { requireCached: true })`) re-checks `isCached` under the alias's residency scope before admission, because a deletion can land between resolution and load and admission may unload the resident variant. The same check guards reloading a pool entry the runtime reports as no longer loaded.
...
- The HTTP router is the mirror image: it routes **only the exact, case-sensitive loaded variant id** (`qwen3-0.6b-generic-cpu:4`) and answers `404 Model not found` to the alias, the versionless id, and any other casing even while that model is resident (SDK 2.0.1; 1.x also accepted the versionless form). The gateway therefore rewrites the replayed body to the variant id the loader reports, and caches that mapping. A request must never be replayed under the client's own alias.
...
- SDK 2.0.1 fixes the catalog on first access and no-argument EP registration selects only one preferred provider. Register every discovered EP explicitly before the first catalog read; otherwise aliases without that provider's build silently lose their other GPU variants for the process. The sidecar single-flights that registration and serializes the first catalog read with it, because a list, load, or gateway resolve can arrive before startup finishes registering. Automatic `poolStatus` polling must not become the first catalog access: it skips `getLoadedModels()` until a queued `getModels()` read confirms the snapshot, then uses a telemetry lane that bypasses later restart-bound EP updates but waits for an actively running local catalog mutation. This keeps `poolStatus` within its finite transport deadline while catalog-gated queries stay unbounded. A listener exposed with the registry read deferred leaves the snapshot uncertain, so later EP updates are deferred until a queued read confirms it or Flint restarts. Provider-sensitive reads are tracked when queued, not only when their operation begins, so a post-commit registration rerun waits for lookups queued during the catalog transition. Registration is retried a few times before the read: an empty discovery or a failed provider download must not be kept as the final set, and a retryable failure carried from an earlier attempt keeps retrying when a later attempt does not re-evaluate that provider (for example, it dropped out of discovery). The no-argument fallback selects one provider and reports success; any provider visible after that call is registered explicitly before the result is kept. Import, link, template edits, and deletion serialize with provider registration before touching the catalog. When a deletion refresh fails after the durable mutation, the SDK reconciles the known cache and residency change locally, re-deriving the loaded flags and both lanes from the reconciled pool, and returns restart guidance rather than publishing stale deletion state or reporting a false operation failure. `startService` waits on the same registration before `startWebService`, because the native listener's `GET /v1/models` is a catalog read the JS calls do not see; that ordering read is network-backed, so its failure is logged and the service still starts. Registration progress rides the requesting command's IPC id tagged `phase: 'accelerator'`, and only accelerator-aware callbacks receive it — a model download or load must never display provider percentages as its own progress, though the stall watchdog still counts them.
...
- The pool records native residency, which survives a listener restart: the new listener still serves a loaded build by its exact id. `startService` therefore keeps the pool and usage; forgetting them would let an exact-id request bypass activity fences and let deletion remove a build beneath it.
- Per-alias model operations use separate cache and residency scopes: downloads may coexist with inference/load checks, while deletion acquires both scopes and cannot race an admitted download, load, or unload.
- Activity accounting is also request admission: destructive unload, variant-switch, eviction, and deletion hold a synchronous per-alias activity fence for their whole destructive window. Gateway and IPC inference must acquire activity before loading or creating temporary state; a point-in-time `inFlight` check is not a fence. Bookings (`sidecar/model-activity-fence.js`) are keyed by the normalized requested name, so each end releases exactly what its start booked however the pool changes meanwhile. An alias's `inFlight` counts only bookings served by its resident build (the alias, or a name the pool resolves to it); a request for another variant is not counted, so it cannot block its own variant switch. The fence denies a name only when it, its pool alias, or its catalog alias is fenced; unresolved names are admitted and later serialize on the residency scope.
...
- Proxied requests never reach the sidecar's chat path, so the pool cannot see them. `createGateway({onActivity})` keeps each whole exchange (buffer → forward → autoload → replay) continuously booked; without it a model streaming a long completion looks idle and can be evicted mid-generation. The booking names what is forwarded: when the gateway rewrites to a variant id (a learned rewrite, or the variant the loader reports) it books the new name before ending the old, so one exchange may report several start/end pairs. The native router serves only the exact loaded variant id, so the forwarded name identifies the serving build even when the requested version fell back to another cached build.
- Gateway responses buffered for `/status` rewriting or first-pass autoload-error inspection are capped before concatenation and must complete within five seconds. Declared or streamed overflow, or capture timeout, returns 502 and cannot trigger autoload; bodyless HTTP responses are not rejected solely for a declared length above the cap. Ordinary and replayed inference responses remain streamed and uncapped.
- Gateway request replay and buffered response limits must be finite, non-negative numbers and are normalized once at construction. An undeclared request body may equal the cap; exceeding it returns 413 without forwarding a truncated body.
...
- SDK 2.0.1's `stopWebService()` sends the native stop only when `manager.urls` is non-empty, and the native manager is private. If startup may have opened a listener but URL publication failed, call the public stop method but report teardown as unconfirmed; never invent a private fallback or claim the listener stopped.
...
- Provider discovery is ordered behind earlier provider registration, but once the first registration has settled it is not ordered behind unrelated catalog mutations; before then it waits for the whole startup registration queue. Because provider installation is unbounded, `getEps` is unbounded too and uses a 60-second no-progress notice rather than discarding a late result. Automatic accelerator setup is single-flight; an explicit Install / Update request waits for the active cycle and then performs a fresh rerun, observing the active cycle's progress and stall notice while it waits. Callers that pass no `onStall` still get the default quiet-period log notice, emitted once per stall.
...
- Native chat/transcription HTTP error diagnostics are capped at 64 KiB with a five-second body deadline; timeout or truncation is explicit in the preserved HTTP failure. Successful inference and transcription responses are not subject to that diagnostic cap.
...
- Catalog telemetry has a 10-second sidecar wait budget inside the 20-second IPC deadline. Queued expiry revokes dispatch; an expired dispatched read keeps native exclusion until it settles, while catalog mutations fail before starting instead of waiting forever. This cannot interrupt a synchronous native call blocking Node's event loop.
- Loaded-model projection excludes pool entries explicitly reported `isLoaded: false`, retaining them only for monitoring. Unknown telemetry preserves last-known residency; evicted legacy lane positions are cleared, not reassigned.
- Page residency decisions use `loadedPoolEntries`, filtered through the SDK's shared `isPoolEntryResident` predicate. Raw `state.pool` is monitoring data, not proof a variant is loaded; keep actions, Arena, badges, startup capture, and memory accounting on the filtered view.
- Unknown pool telemetry preserves the previous explicit residency verdict only for the same alias and variant ID; it must not resurrect a confirmed eviction. Explicit new verdicts win, and new identities remain unknown. A live-generation load acknowledgement records the returned variant as loaded before refreshing, so unknown telemetry cannot preserve a superseded eviction.
- Downloads and accelerator registration remain unbounded but surface a notice after 60 seconds without reported progress. Start the watchdog only at dispatch, reset it on progress, and retire it on every reply, cancellation, drain, or reset. The notice says Flint is still awaiting the runtime and has not cancelled; it does not claim the native work is still running.
...
- A model must be **loaded first** — otherwise requests for its exact variant id fail `400 Model not loaded: Model 'X' must be loaded before inference` (SDK 1.x: `Model 'X' is not loaded`). Load via `catModel.load()` (there is no `manager.loadModel()`).
