...
- SDK 2.0.1 fixes the catalog on first access and no-argument EP registration selects only one preferred provider. Register every discovered EP explicitly before the first catalog read; otherwise aliases without that provider's build silently lose their other GPU variants for the process. The sidecar single-flights that registration and every catalog read waits on it, because a list, load, gateway resolve, or pool status can arrive before startup finishes registering. That attempt is retried a few times before the read: an empty discovery or a failed provider download must not be kept as the final set. The no-argument fallback selects one provider and reports success; any provider visible after that call is registered explicitly before the result is kept. Import, link, and template edits wait on the same registration before touching the catalog. `startService` waits on the same registration before `startWebService`, because the native listener's `GET /v1/models` is a catalog read the JS calls do not see.
